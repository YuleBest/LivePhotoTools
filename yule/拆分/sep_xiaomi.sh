...
getLivePhotoDir() {
    ...
    if [[ $videoOffset == '' ]]; then
        ee "${bre}文件${livePhotoDir}可能不是小米动态照片格式，请重新输入${ba}"
        getLivePhotoDir
    fi
    main
}
...
getLivePhotoDirD() {
    ...
    if [[ ${#livePhotoFiles[@]} -eq 0 ]]; then
        ee "${bre}目录${livePhotoDirD}中没有找到任何小米动态照片文件，请重新输入${ba}"
        getLivePhotoDirD
    fi
...
processFile() {
    ...
    if [[ $videoOffset == '' ]]; then
        ee "${bre}文件${file}可能不是小米动态照片格式，跳过${ba}"
